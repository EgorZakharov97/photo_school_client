      ...
      <span>
        ...
        <span className="af-view">
          <form id="wf-form-courseForm" enctype="multipart/form-data" name="wf-form-courseForm" data-name="courseForm" method="post" className="af-class-form-5"><label htmlFor="workshopName">Select workshop</label>{map(proxies['selector'], props => <select id="workshopName" name="workshopName" data-name="workshopName" {...{...props, className: `w-select ${props.className || ''}`}}>{props.children ? props.children : <React.Fragment><option value="!new">New</option></React.Fragment>}</select>)}<label htmlFor="name">Workshop Name</label>{map(proxies['name'], props => <input type="text" maxLength={256} name="name" data-name="Name" placeholder="Workshop Name" id="name" required {...{...props, className: `w-input ${props.className || ''}`}}>{props.children}</input>)}<label htmlFor="starts">Date the workshop starts</label>
            ...
            {map(proxies['deadline'], props => <div {...{...props, className: `af-class-html-embed-4 w-embed ${props.className || ''}`}}>{props.children ? props.children : <React.Fragment><input type="date" name="deadline" placeholder="Registration deadline" required /></React.Fragment>}</div>)}<label htmlFor="duration">Duration</label>{map(proxies['duration'], props => <input type="number" maxLength={256} name="duration" data-name="duration" placeholder="Duration in hrs" id="duration" required {...{...props, className: `w-input ${props.className || ''}`}}>{props.children}</input>)}<label htmlFor="price-2">Price</label>{map(proxies['price'], props => <input type="text" maxLength={256} name="price" data-name="price" placeholder="Price" id="price-2" required {...{...props, className: `w-input ${props.className || ''}`}}>{props.children}</input>)}<label>Number of seats</label>{map(proxies['seats'], props => <input type="number" maxLength={256} name="seats" data-name="Num Places" placeholder="Number of places" id="numPlaces" required {...{...props, className: `w-input ${props.className || ''}`}}>{props.children}</input>)}<label>Workshop preview image</label>
            ...
            <div className="af-class-div-block-38">{map(proxies['submit'], props => <input type="submit" value="Create" data-wait="Please wait..." id="submit" {...{...props, className: `af-class-button af-class-red w-button ${props.className || ''}`}}>{props.children}</input>)}
