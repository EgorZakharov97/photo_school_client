    ...
    return (
      <span>
        ...
        <span className="af-view">
          <form id="wf-form-material-form" name="wf-form-material-form" data-name="material form" encType="multipart/form-data" className="af-class-form-6"><label htmlFor="name-2">Select Material</label>{map(proxies['selector'], props => <select id="tutorial-2" name="tutorial-2" data-name="Tutorial 2" {...{...props, className: `w-select ${props.className || ''}`}}>{props.children ? props.children : <React.Fragment><option value="!new">New</option></React.Fragment>}</select>)}<label htmlFor="name-9">Material Name</label>{map(proxies['name'], props => <input type="text" maxLength={256} name="name" data-name="name" placeholder="Material name" id="name-9" required {...{...props, className: `w-input ${props.className || ''}`}}>{props.children}</input>)}<label htmlFor="embed-3">Access only with subscription</label><label className="w-checkbox">{map(proxies['subscription'], props => <input type="checkbox" id="subscription-2" name="subscription-2" data-name="Subscription 2" {...{...props, className: `w-checkbox-input ${props.className || ''}`}}>{props.children}</input>)}<span htmlFor="subscription-2" className="w-form-label">Requires to have subscription</span></label>
