    ...
    return (
      <span>
        ...
        <span className="af-view">
          <form id="email-form-2" name="email-form-2" data-name="Email Form 2" encType="multipart/form-data" className="af-class-form-8">
            {map(proxies['info'], props => <div {...{...props, className: `af-class-info ${props.className || ''}`}}>{createScope(props.children, proxies => <React.Fragment><label htmlFor="selecrot">Select Course</label>{map(proxies['selector'], props => <select id="selecrot" name="selecrot" data-name="selecrot" {...{...props, className: `w-select ${props.className || ''}`}}>{props.children ? props.children : <React.Fragment><option value="new">New</option></React.Fragment>}</select>)}<label htmlFor="name-7">Course name</label>{map(proxies['name'], props => <input type="text" maxLength={256} name="name" data-name="name" placeholder="Course name" id="name-7" required {...{...props, className: `w-input ${props.className || ''}`}}>{props.children}</input>)}<label htmlFor="price-3">Price</label>{map(proxies['price'], props => <input type="number" maxLength={256} name="price" data-name="price" placeholder="Price" id="price-3" required {...{...props, className: `w-input ${props.className || ''}`}}>{props.children}</input>)}<label htmlFor="description-7">Short Description</label>{map(proxies['description'], props => <input type="text" maxLength={256} name="description" data-name="description" placeholder="Short Description" id="description-7" required {...{...props, className: `w-input ${props.className || ''}`}}>{props.children}</input>)}<label htmlFor="description-8">Choose Preview Image</label>
              {map(proxies['image'], props => <div {...{...props, className: `af-class-html-embed-3 w-embed ${props.className || ''}`}}>{props.children ? props.children : <React.Fragment><input type="file" name="image" placeholder="Choose Image" /></React.Fragment>}</div>)}<label htmlFor="description-5">Publish</label><label className="w-checkbox">{map(proxies['access'], props => <input type="checkbox" id="access" name="access" data-name="access" {...{...props, className: `w-checkbox-input ${props.className || ''}`}}>{props.children}</input>)}<span htmlFor="access" className="w-form-label">This course can be accessed by users</span></label><label htmlFor="description-8">Dummy</label><label className="w-checkbox">{map(proxies['dummy'], props => <input type="checkbox" id="dummy" name="dummy" data-name="dummy" {...{...props, className: `w-checkbox-input ${props.className || ''}`}}>{props.children}</input>)}<span htmlFor="dummy" className="w-form-label">This course does not have content</span></label>
                ...
                <div className="af-class-table-container">
                  ...
                  {map(proxies['examples-container'], props => <div {...{...props, className: `af-class-table-element af-class-image ${props.className || ''}`}}>{createScope(props.children, proxies => <React.Fragment>
                    ...
                    <div className="af-class-table-element-image af-class-new">
                      <div className="af-class-div-block-40">{map(proxies['upload'], props => <a href="#" {...{...props, className: `af-class-button af-class-update w-button ${props.className || ''}`}}>{props.children ? props.children : <React.Fragment>Save</React.Fragment>}</a>)}</div>
